...
const $=id=>document.getElementById(id);let episodes=[];let current=null;let selectedEpisodeId=null;let timer=null;let playbackBusy=false;let signalSeries=null;
...
async function refreshAll(){try{const [h,f]=await Promise.all([json('/api/health'),json('/api/frames')]);if(!timer){image('overheadImage',f.overhead);image('wristImage',f.wrist);image('inspectionImage',f.inspection)}$('healthPills').innerHTML=`<span class="pill ok">sim ${h.time.toFixed(2)} s</span><span class="pill">components ${h.collected}/${h.components}</span><span class="pill">Fz ${h.normal_force.toFixed(2)} N</span><span class="pill">${h.model}</span>`;await loadEpisodes();await refreshJobs()}catch(e){$('globalStatus').textContent='Dashboard error: '+e.message;$('globalStatus').className='bad'}}
...
async function loadSignals(id){try{signalSeries=await json(`/api/episodes/${encodeURIComponent(id)}/signals`)}catch(e){signalSeries=null}renderFz(0)}
