...
overhead RGB + wrist RGB
        ...
        -> binary table-topology map (identity-free by default)
...
signed distance. The first three occupancy channels are hard binary maps at
the policy boundary. By default ObjectACT sets selected occupancy to zero and
unselected occupancy to all occupancy: offline final-collected track IDs
remain audit metadata, not a policy input. The learned Top-N selector and its
selected/unselected split are retained only as an explicit ablation, because
the task accepts any exact N complete objects rather than a fixed identity set.
The token fields include grouped class probabilities, current and previous
XY/extent/yaw features, velocity, brush/tray-relative geometry, visibility,
confidence, staleness and validity.
...
4. Report both raw mask diagnostics and the temporal centre/track metrics used
   ...
   Pixel thresholding of the raw RGB image is explicitly not the runtime
   contract: the checkerboard, shadows, robot and brush can all change
   appearance. The Push-Wiper-style binary representation is applied after
   the learned RGB objectness/instance decoder and calibrated XY projection.
   A fixed empty-table calibration may be added later as a causal auxiliary
   feature, but it cannot replace the learned RGB objectness gate or introduce
   simulator truth.
...
- A* may generate the high-quality expert action path, but target identities
  never become policy inputs. The v5 manifest retains final-collected IDs only
  for audit and selection-head ablations; the default policy receives the
  all-object binary topology and target cardinality. The v5 manifest accepts
  only successful expert records with action dimension 4.
...
- Add a chunk-level cumulative action loss alongside per-step L1 so an action
  block is penalized for net XY/yaw drift, not only for framewise error. This
  is intended to suppress aimless left-right oscillation while preserving the
  original four-dimensional action contract.
