...
<section class="panel inference"><h2>Inference <span class="muted">· MuJoCo rollout / replay</span></h2>
<label>Policy variant<select id="inferVariant" onchange="toggleInferenceVariant()"><option value="objectact" selected>ObjectACT-BEV · v5</option><option value="ordinary">ordinary ACT · v4</option></select></label><label>Model checkpoint<input id="inferModel" value="runs/objectact_model/checkpoints/step_080000"></label><label>Exact target count (of 6)<select id="inferTarget"><option>1</option><option>2</option><option selected>3</option><option>4</option><option>5</option><option>6</option></select></label><label>Seed<input id="inferSeed" type="number" value="0"></label><label class="check"><input id="inferRandomize" type="checkbox" checked> Randomize layout seed on each run</label><label class="check"><input id="inferPreview" type="checkbox" checked> Simulation preview · allow late inference</label>
...
function toggleInferenceVariant(){const objectact=$('inferVariant').value==='objectact';const current=$('inferModel').value.trim();if(objectact&&(!current||current==='runs/act_model'))$('inferModel').value='runs/objectact_model/checkpoints/step_080000';if(!objectact&&(!current||current.startsWith('runs/objectact_model')))$('inferModel').value='runs/act_model'}
...
def create_app(cfg):
    ...
    def inference(payload: dict):
        variant = str(payload.get("variant", "objectact")).lower()
        if variant not in {"ordinary", "objectact"}:
            raise HTTPException(
                status_code=422,
                detail="variant must be ordinary or objectact",
            )
        try:
        ...
        argv = build_inference_argv(project_root,
                                    ...
                                    target_count, preview=preview, variant=variant)
        try:
