...
const $=id=>document.getElementById(id);let episodes=[];let current=null;let timer=null;
...
async function makePreview(){try{$('globalStatus').textContent='Generating a fresh expert preview in MuJoCo…';const e=await json('/api/preview',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({seed:Date.now()%100000,count:null})});await loadEpisodes();$('episodeSelect').value=e.episode_id;await selectEpisode(e.episode_id);$('globalStatus').textContent=`Preview ${e.episode_id} ready; formal dataset unchanged.`}catch(e){$('globalStatus').textContent='Preview failed: '+e.message}}
